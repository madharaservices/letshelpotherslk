...
import OfferForm from '@/components/OfferForm'; // <-- අලුත් Form එක Import කළා
import Footer from '@/components/Footer';
import { Plus, AlertTriangle, Heart, MapPin } from 'lucide-react';

export default function Home() {
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [isOfferFormOpen, setIsOfferFormOpen] = useState(false); // <-- අලුත් State එකක්
  ...
  const Map = useMemo(() => dynamic(() => import('@/components/RealtimeMap'), { 
      loading: () => <div className="h-[400px] w-full bg-slate-100 rounded-3xl animate-pulse flex items-center justify-center text-slate-400">සිතියම සකසමින්...</div>,
      ssr: false 
  }), []);
    ...
    <div className="min-h-screen bg-slate-50 font-sans selection:bg-blue-200">
      ...
      <div className="bg-gradient-to-br from-blue-950 to-slate-900 text-white px-4 py-10 mb-6 rounded-b-[2.5rem] shadow-xl shadow-blue-900/20">
        <div className="max-w-4xl mx-auto text-center space-y-5">
          ...
          <div className="flex justify-center gap-3 mt-6">
            ...
            {/* උදව් කරන්න Button එක දැන් වැඩ */}
            <button onClick={() => setIsOfferFormOpen(true)} className="flex items-center gap-2 px-6 py-3.5 bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/10 rounded-xl font-bold text-sm active:scale-95 transition-all text-white">
      ...
      <div className="max-w-4xl mx-auto px-4 space-y-8">
        <div className="bg-white p-2 rounded-3xl shadow-sm border border-slate-100">
          <div className="px-4 py-3"><h3 className="font-bold text-slate-800 flex items-center gap-2"><MapPin className="w-4 h-4 text-blue-600" />ආපදා සිතියම (Live Map)</h3></div>
          <Map />
        </div>

        {/* List එකේ දැන් Tabs වැඩ */}
      ...
      {/* අලුත් Offer Form Modal එක */}
      {isOfferFormOpen && <OfferForm onClose={() => setIsOfferFormOpen(false)} onSuccess={handleSuccess} />}
    </div>
  );
}